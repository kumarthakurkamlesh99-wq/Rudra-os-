...
import com.example.notification.NotificationPreferencesRepository
...
class NotificationSettingsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val notifService = NotificationManagerService(application)
    ...
    val permissionPromptShown: StateFlow<Boolean> = repository.permissionPromptShown
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), false)

    fun setNotificationsEnabled(enabled: Boolean) {
        viewModelScope.launch {
            repository.setNotificationsEnabled(enabled)
            val prefs = com.example.data.preferences.UserPreferences(getApplication())
            scheduler.rescheduleAllRoutineAlarms(prefs)
        }
    }
    ...
    fun setRevisionRemindersEnabled(enabled: Boolean) {
        viewModelScope.launch {
            repository.setRevisionRemindersEnabled(enabled)
        }
    }

    fun setTaskRemindersEnabled(enabled: Boolean) {
        viewModelScope.launch {
            repository.setTaskRemindersEnabled(enabled)
        }
    }
    ...
    fun setRecoveryRemindersEnabled(enabled: Boolean) {
        viewModelScope.launch {
            repository.setRecoveryRemindersEnabled(enabled)
        }
    }
    ...
    fun setPermissionPromptShown(shown: Boolean) {
        viewModelScope.launch { repository.setPermissionPromptShown(shown) }
    }

    fun triggerTestNotification() {
        notifService.showTestNotification()
    }
}
